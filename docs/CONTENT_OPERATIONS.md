...
The Website Submissions compact workbook formatting, secure idempotent form intake, and bridge-managed notification emails require the deployed Apps Script bridge to report `3.3.0`. If the bridge reports an older version, deploy the current `scripts/website-bridge-apps-script.js` file first, then rerun:
