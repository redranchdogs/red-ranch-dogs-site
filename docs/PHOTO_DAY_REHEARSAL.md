# Photo Day Rehearsal

Generated: 5/19/2026, 8:28:32 PM Central

Status: **READY**

This is the pre-photo-day rehearsal for current litters. It does not move files, edit Google Drive, write Sheets, or publish website changes. It tells us what the next photo import should do.

## Current Litter Readiness

| Litter | Latest website photos | Next folder | Puppies | Main-photo needs | Weekly-group needs |
| --- | --- | --- | --- | --- | --- |
| Birdie + Waylon | Week 5 | Website Hub / Weekly Photo Drops / Birdie 3 + Waylon / Week 6 | 7 | 0 | 0 |
| Whitley + Waylon | Newborn Photos | Website Hub / Weekly Photo Drops / Whitley 1 + Waylon / Week 1 | 6 | 6 | 6 |
| Penny + Wyatt | Newborn Photos | Website Hub / Weekly Photo Drops / Penny 1 + Wyatt / Week 1 | 6 | 0 | 0 |
| Ginny + Butch Cassidy | Newborn Photos | Website Hub / Weekly Photo Drops / Ginny 1 + Butch Cassidy / Week 1 | 6 | 0 | 0 |

## Puppy-Level Import Actions

| Litter | Puppy | Collar | Status | Import action |
| --- | --- | --- | --- | --- |
| Birdie + Waylon | Ranger | Brown | Reserved | Add Week 6 group |
| Birdie + Waylon | Axel | Orange | Reserved | Add Week 6 group |
| Birdie + Waylon | Diesel | Blue | Reserved | Add Week 6 group |
| Birdie + Waylon | Dakota | Red | Reserved | Add Week 6 group |
| Birdie + Waylon | Sedona | Yellow | Reserved | Add Week 6 group |
| Birdie + Waylon | Arizona | Pink | Reserved | Add Week 6 group |
| Birdie + Waylon | Indie | Purple | Reserved | Add Week 6 group |
| Whitley + Waylon | Hook | Blue | Waitlist Matching | Choose main photo and create first weekly group |
| Whitley + Waylon | Minnow | Pink | Waitlist Matching | Choose main photo and create first weekly group |
| Whitley + Waylon | Tackle | Orange | Waitlist Matching | Choose main photo and create first weekly group |
| Whitley + Waylon | Bass | Grey | Waitlist Matching | Choose main photo and create first weekly group |
| Whitley + Waylon | Bobber | Maroon | Waitlist Matching | Choose main photo and create first weekly group |
| Whitley + Waylon | Skipper | Black | Waitlist Matching | Choose main photo and create first weekly group |
| Penny + Wyatt | Striker | Orange | Waitlist Matching | Add Week 1 group |
| Penny + Wyatt | Samba | Dark Blue | Waitlist Matching | Add Week 1 group |
| Penny + Wyatt | Goalie | Dark Gray | Waitlist Matching | Add Week 1 group |
| Penny + Wyatt | Ref | Black | Waitlist Matching | Add Week 1 group |
| Penny + Wyatt | Pitch | Red | Waitlist Matching | Add Week 1 group |
| Penny + Wyatt | FIFA | Light Gray | Waitlist Matching | Add Week 1 group |
| Ginny + Butch Cassidy | Meadow | Red | Waitlist Matching | Add Week 1 group |
| Ginny + Butch Cassidy | Vista | Maroon | Waitlist Matching | Add Week 1 group |
| Ginny + Butch Cassidy | Camper | Gray | Waitlist Matching | Add Week 1 group |
| Ginny + Butch Cassidy | Sage | Pink | Waitlist Matching | Add Week 1 group |
| Ginny + Butch Cassidy | Ridge | Orange | Waitlist Matching | Add Week 1 group |
| Ginny + Butch Cassidy | Trek | Black | Waitlist Matching | Add Week 1 group |

## Rehearsal Notes

- No folder blockers flagged.
- NOTE: Whitley + Waylon: 6 puppies need main photos.
- NOTE: Whitley + Waylon: 6 puppies need first weekly groups.

## After Photo Day

```bash
npm run photos:packet
npm run sync:puppies
npm run sync:litters
npm run review:sheets
npm run publish:check
```
