...
const reportPath = path.join(root, "docs", "CRM_WEBSITE_INTEGRATION_CHECKLIST.md");
const generatedAt = new Date().toLocaleString("en-US", { timeZone: "America/Chicago" });

const report = `# CRM Website Integration Checklist
...
5. Test with UAT/test-delete rows before importing real customer records.
...
console.log(`CRM website integration checklist written to ${path.relative(root, reportPath)}`);
